...
def create_token(username: str, name: str, role: str) -> str:
    ...
    return jwt.encode(
        {"sub": username, "name": name, "role": role, "exp": expire},
        SECRET_KEY,
        algorithm=ALGORITHM,
    )
...
def create_user(username: str, name: str, password: str, role: str) -> dict:
...
def update_user(username: str, name: Optional[str] = None, password: Optional[str] = None, role: Optional[str] = None) -> Optional[dict]:
    ...
    for u in users:
        if u["username"] == username:
            ...
            if role is not None:
                u["role"] = role
            save_users(users)
            return {"username": u["username"], "name": u["name"], "role": u.get("role", "franqueado")}
    return None
